...
#
# Preferred daily wrappers:
#   ./scripts/run_feature_export_predict.sh  # Gazebo semantic reference CSV
#   ./scripts/run_feature_export_rf.sh       # RF-predicted CSV
#
...
# Optional C++ Random Forest text model generated by python/train_model.py.
# When set, Octree leaf labels and obstacle probabilities are predicted by
# the model during Octree construction.
# Example: FEATURE_RF_MODEL=models/random_forest_voxel_model.rf.txt
: "${FEATURE_RF_MODEL:=}"
...
Environment overrides:
  ...
  FEATURE_RF_MODEL        Optional C++ RF model path for Octree leaf inference
...
if [ -n "${FEATURE_RF_MODEL}" ]; then
  args+=(--rf-model "${FEATURE_RF_MODEL}")
fi
