...
# When set, the Octree viewer renders model-predicted labels/probabilities.
# Example: OCTREE_RF_MODEL=models/random_forest_voxel_model.rf.txt
: "${OCTREE_RF_MODEL:=}"
...
EOF
    ...
    if [ -n "${OCTREE_RF_MODEL}" ]; then
      octree_args+=(--rf-model "${OCTREE_RF_MODEL}")
    fi
